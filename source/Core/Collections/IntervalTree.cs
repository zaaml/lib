...
{
	internal sealed class IntervalTree<TItem, T> where T : IComparable<T>
	{
		...
		internal IntervalTree(IEnumerable<IntervalItem<TItem, T>> items)
		{
		...
		private class Node
		{
			public Node(ICollection<IntervalItem<TItem, T>> items)
			{
				...
				Items = new List<IntervalItem<TItem, T>>();

				var left = new List<IntervalItem<TItem, T>>();
				var right = new List<IntervalItem<TItem, T>>();

				foreach (var item in items)
				{
					var interval = item.Interval;

					if (interval.Maximum.CompareTo(Center) < 0)
						left.Add(item);
					else if (interval.Minimum.CompareTo(Center) > 0)
						right.Add(item);
					else
						Items.Add(item);
				}
			...
			private List<IntervalItem<TItem, T>> Items { get; }
			...
			public void Query(T value, ICollection<TItem> result)
			{
				if (Items != null)
				{
					...
					for (var index = 0; index < itemsCount; index++)
					{
						...
						if (interval.Minimum.CompareTo(value) > 0)
							break;
	...
	{
		public static IntervalTree<TItem, T> Build<TItem, T>(IEnumerable<TItem> items, Func<TItem, Interval<T>> intervalFactory) where T : IComparable<T>
		{
			return new IntervalTree<TItem, T>(items, intervalFactory);
		}
	}
}